import { FindOptions, AddonData, SearchBody, Collection, SearchData } from "@pepperi-addons/papi-sdk";
...
export class ApiService extends IApiService {
    ...
    async search(collectionName: string, params: SearchBody): Promise<SearchData<AddonData>> {
        return (await pepperi.addons.data.uuid(config.AddonUUID).table(collectionName).search(params));
    }
